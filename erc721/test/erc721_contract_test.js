...
contract('ERC721', ([owner, userOne, userTwo , userThree]) => {
    ...
    before(async () => {
        ...
        erc721 = await ERC721.new('ERC721','XOXO')
    ...
    describe('Mint Token And Verify', async () => {
        ...
        it('transfer token 2 from userOne to userTwo',async  () => {
            ...
            await erc721.safeTransferFrom(userOne ,userTwo ,2, bytes, {
                from: userOne
            })
        })
        ...
        it('transfer token 2 from userTwo to userThree by userThree',async  () => {
            const bytes = web3.utils.asciiToHex("TEST");
            await erc721.safeTransferFrom(userTwo ,userThree ,2, bytes, {
                from: userThree
            })
        })
        ...
        it('transfer token 4 from userOne to userTwo by userTwo',async  () => {
            const bytes = web3.utils.asciiToHex("TEST");
            await erc721.safeTransferFrom(userOne ,userTwo ,4, bytes, {
                from: userTwo
            })
        })
        ...
        it('Verify Contract Transfer', async () => {
            const bytes = web3.utils.asciiToHex("TEST");
            await erc721.safeTransferFrom(userTwo ,receiverContract ,4, bytes, {
                from: userTwo
            })
        })
